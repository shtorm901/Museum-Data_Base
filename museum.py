...
with connection:
    try:
        ...
        cursor.execute('INSERT INTO workers(Post_id, named, surname, middle_name, telephone_number) VALUES ("2","Aleksei", "Kamarov", "Ivanovich", "None"), ("1","Michail", "Morozov", "Alekseevich", "+7(999(617-19-02"), ("3", "Nikolay", "Semenov", "Aleksandrovich", "None"), ("3", "Anna", "Leonteva", "Andreevna", "+7(937)225-02-37")')
        cursor.execute(table_post)
        cursor.execute('INSERT INTO post(Post) VALUES ("Уборщик"), ("Охранник"), ("Экскурсовод"), ("Заведущющий музеем")')
        cursor.execute(table_hall)
        cursor.execute('INSERT INTO hall(Title_Hall, Floor, Square) VALUES ("бытовые предметы времен второй мировой войны", "1", "1"), ("оружие времен второй мировой войны", "2", "3"), ("одежды времен второй мировой войны", "3", "2")')
        cursor.execute(table_exhibition_exhibits)
        cursor.execute('INSERT INTO exhibition_exhibits(Hall_id, Title_Exhibit, Date_of_discovery) VALUES ("2", "Пулемет дегтярева", "2014"), ("1", "Открытка из госпиталя", "2012")')
        cursor.execute(table_excursion)
        cursor.execute('INSERT INTO excursion(Floor, Square, Exhibit_id, Workers_id, Number_of_People) VALUES (1, 1, 2, 4, 50), (3, 2, 1, 3, 50)')
        connection.commit()
